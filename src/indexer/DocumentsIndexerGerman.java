...
public class DocumentsIndexerGerman {
    ...
    private DocumentsIndexerGerman(String propFile) throws Exception {
        prop = new Properties();
    ...
    private DocumentsIndexerGerman(String _separator, int _docContentIdx, int _docDomainIdx, String indexPath, String _coll) throws Exception {
        prop = new Properties();
        prop.setProperty("coll", _coll);
        prop.setProperty("stopfile", "./sw_german.txt");
        analyzer = constructAnalyzer();
        indexDir = new File(indexPath);
...
//        System.out.println("Usage: java DocumentsIndexer <prop-file>");
        ...
        String dataset_path[] = {"./dataset/German"};
        String indexes_path[] = {"./index/German"};
        String separator = ",";
        int domain_pos = 0, content_pos = 1;
        for (int i = 0; i < dataset_path.length; i++) {
            try {
                DocumentsIndexerGerman indexer = new DocumentsIndexerGerman(separator, content_pos, domain_pos, indexes_path[i], dataset_path[i]);
                indexer.processAll();
            }
            catch (Exception ex) {
                ex.printStackTrace();
